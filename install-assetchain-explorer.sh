...
if [ ! -d "$CUR_DIR/explorer-notarized" ]; then
  ...
  while [ $success -eq 0 ]; do
    ...
    git clone https://github.com/NATURE-SMARTCHAIN/explorer-notarized && success=1 || success=0
    sleep 4
    count=$((count+1))
  done
else
  echo "A directory named 'explorer-notarized' already exists; assuming it is cloned from the repo: https://github.com/nature-smartchain/explorer-notarized , trying to update and patch the explorer using it"
